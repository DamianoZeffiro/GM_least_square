...
from VR_STGM_ls import VR_STGM_ls
...
nepochs = 10000
xagm, iteragm, fxagm, tottimeagm, fhsvrgm, timeVecsvrgm, gnragm = VR_STGM_ls(Q, c, x1, lsvrg, verb, nepochs, maxit3, eps, fstop, stopcr)
...
plt.semilogy(timeVecsvrgm, fhsvrgm - fmin, 'g-', label='SVRG')
plt.semilogy(timeVecrgm, fhrgm - fmin, 'y-', label='RGM')

plt.title('GDvsSGDvsSVRGvsRGM - objective function')
plt.legend()
plt.xlabel('time')
plt.ylabel('err')
plt.show()
